...
def test_exercises_add(driver: webdriver.Chrome) -> None:
    ...
    expected_current = {e.name for e in USER.exercises if e.name in CURRENT_WORKOUTS_EXERCISES}
...
def test_exercises_edit(driver: webdriver.Chrome) -> None:
    current_name = sorted(CURRENT_WORKOUTS_EXERCISES)[0]
...
def test_exercises_delete(driver: webdriver.Chrome) -> None:
    current_exercises = sorted(CURRENT_WORKOUTS_EXERCISES)
