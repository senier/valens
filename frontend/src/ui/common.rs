...
impl Bounds {
    fn y_margin(self) -> f32 {
        if self.omit_y_margin {
            return 0.;
        }
        if (self.max - self.min).abs() > f32::EPSILON || self.min == 0. {
            return (self.max - self.min) * 0.01;
        }
        0.1
    }
}
...
pub fn plot_chart(
...

    {
        ...
        let mut chart = ChartBuilder::on(&root)
            ...
            .build_cartesian_2d(
                x_min..x_max,
                primary_bounds.min - primary_bounds.y_margin()
                    ..primary_bounds.max + primary_bounds.y_margin(),
            )?
            .set_secondary_coord(
                x_min..x_max,
                secondary_bounds
                    .as_ref()
                    .map_or(0.0..0.0, |b| b.min - b.y_margin()..b.max + b.y_margin()),
            );
...
fn determine_y_bounds(data: &[PlotData]) -> (Option<Bounds>, Option<Bounds>) {
    ...
    for plot in data {
        ...
        .get_or_insert(Bounds {
            min,
            max,
            omit_y_margin: plot.params.omit_margin,
        });

        b.min = f32::min(b.min, min);
        b.max = f32::max(b.max, max);
        b.omit_y_margin = b.omit_y_margin || plot.params.omit_margin;
    }
...
pub fn centered_moving_grouping(
    ...
    for elem in data {
        date_map.entry(&elem.0).or_default().push(elem.1);
    }

    let mut grouped: BTreeMap<&NaiveDate, f32> = BTreeMap::new();

    for elem in date_map {
        if let Some(result) = group_day(elem.1) {
            grouped.insert(elem.0, result);
        }
    }
